...
const PropertyApproval = ({ setOpenModal, propertyId }) => {
  ...
  const defaultFormValue = {
    status: "",
    comment:
      "(COMMENT SAMPLE) After careful consideration, we regret to inform you that your property has not been approved on our platform due to [reason for suspension, e.g., policy violations, pending verification, etc.]. Kindly try again later",
  };
    ...
    try {
      ...
      if (propertyId) {
        const response = await patchData(
          `bookacrib-api-routes/v1/admin/properties/update-property-status?is_active=yes&with[]=company&with[]=initiator&with[]=country&with[]=state&id=${propertyId}`,
          filteredFormData
        );
        if (
              ...
              <div className="flex items-center justify-between p-4 md:px-5 border-b rounded-t ">
                <div>
                  ...
                  <p className="text-gray-600 text-sm font-light">
                    Approve, Decline or Suspend this Property
                  </p>
                </div>
                <button
                  type="button"
                        ...
                        {selectedAction?.action === "approved" && (
                          <div className="mb-8 text-center ">
                            ...
                            <div className="text-gray-500 font-normal max-w-lg mx-auto">
                              It is advisable to also verify the property
                              physically before approved
                            </div>
                          </div>
                        )}
                        <div className="pb-4">
