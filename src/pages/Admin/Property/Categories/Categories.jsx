...
const Categories = () => {
  ...
  const fetchCategoryHandler = (url, search) => {
    ...
    dispatch(fetchCategoriess(fetchUrl));
  };
  ...
  const fetchSingleRole = (category_id) => {
    dispatch(fetchCategories(category_id));
    setEditModal(true);
  };
    ...
    if (category) {
      ...
      try {
        ...
        const result = await dispatch(
          deleteCategories(category?.uuid)
        ).unwrap();
        ...
        if (
          ...
          dispatch(
            fetchCategoriess("/v1/public/categories/list-all-categories")
          );
        }
      } catch (error) {
        console.log(error);
    ...
    try {
      ...
      if (category_id) {
        const response = await patchData(
          `/v1/public/categories/toggle-category-status?id=${category_id}`,
          {
            id: category_id,
          }
        );
  ...
  return (
    <>
      ...
      {openModal && <AddCategoriesModal setOpenModal={setOpenModal} />}
