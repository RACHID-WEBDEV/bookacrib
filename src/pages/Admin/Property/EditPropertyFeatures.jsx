...
function EditPropertyFeatures({
  ...
  // console.log("storeAllFeature:", storeAllFeature);
  ...
  const prevDefaultFeatures = useRef([]);
  ...
  useEffect(() => {
    if (
      ...
      JSON.stringify(defaultFeatures) !==
        JSON.stringify(prevDefaultFeatures.current)
    ) {
      if (
        featureFilter.length > 0 ||
        storeAllFeature.length > 0 ||
        ...
        const featureNames = [
          ...
          ...storeAllFeature,
          ...defaultFeatures,
        ].map((feature) => normalizeFeatureName(feature.name));
        const hasDuplicates =
      ...
      prevDefaultFeatures.current = defaultFeatures;
    }
  }, [featureFilter, storeAllFeature, defaultFeatures, setStoreMainFeature]);
  ...
  const handleDelete = (name) => {
    ...
    const updatedDefaultFeatures = defaultFeatures.filter(
      (feature) => feature.name !== name
    );
    setAttributes(updatedFeatureFilter);
    setStoreAllFeature(updatedStoreAllFeature);
    setdefaultFeatures(updatedDefaultFeatures);

    setStoreMainFeature([
      ...updatedFeatureFilter,
      ...updatedStoreAllFeature,
      ...updatedDefaultFeatures,
    ]);
  };
          ...
          <div className=" px-2 lg:px-4 xl:px-5">
            <form onSubmit={handleSubmit}>
              <div className="">
                ...
                <Label
                  text={
                    "Edit/Create your own feature Multiple features is allowed"
                  }
                />
...
export default EditPropertyFeatures;
