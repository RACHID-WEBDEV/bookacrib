...
const BooksRoles = () => {
  ...
  const { roles, role, loading, error } = useSelector(
  ...
  const fetchEditPermission = (role_id) => {
    dispatch(fetchadminRole(role_id));
  ...
  const handleFeatureDelete = async (role) => {
    ...
    if (role) {
      ...
      try {
        // setLoading(true);
        const result = await dispatch(deleteadminRole(role?.uuid)).unwrap();
