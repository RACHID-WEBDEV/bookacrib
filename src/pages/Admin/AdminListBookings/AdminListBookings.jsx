...
const AdminListBookings = () => {
  const dispatch = useDispatch();
  //   const navigate = useNavigate();

  const { listadminbookings, loadingadminbooking, erroradminbooking } =
    useSelector((state) => state.listbooking);

  console.log("listadminbookings:", listadminbookings);
  //   const fetchCribAdminBookingHandler = (url) => {
  //     dispatch(fetchFeatures(url));
  //   };

  const message = getErrormessage(erroradminbooking);

  if (
    (erroradminbooking?.status >= 400 && erroradminbooking?.status <= 499) ||
    (erroradminbooking?.status_code >= 400 &&
      erroradminbooking?.status_code <= 499 &&
      erroradminbooking?.errors)
  ) {
    toast.error(message);
  }

  //   useEffect(() => {
  //     fetchCribAdminBookingHandler(
  //       "/bookacrib-api-routes/v1/admin/bookings/list-all-bookings?with[]=property&with[]=user?limit=10"
  //     );
  ...
  const fetchCribAdminBookingHandler = (
    url,
    search,
    arrival_status,
    payment_status
    ...
    dispatch(fetchAdminListBookings(fetchUrl));
  };

  useEffect(() => {
    fetchCribAdminBookingHandler(
      "/bookacrib-api-routes/v1/admin/bookings/list-all-bookings?with[]=property&with[]=user"
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch]);

  useEffect(() => {
    const fetchSearchAdminBookings = async () => {
    ...
    fetchSearchAdminBookings();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchQuery]);

  const handlePaginate = (url) => {
    if (url) {
      fetchCribAdminBookingHandler(url);
    }
  };
  ...
  const checkDetails = (record) => {
    return navigate(`/admin/transactions/booking-details/${record.uuid}`);
  };
  const Headings = {
    tableHeadings: [
      "S/N",
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          <div className=" py-2 pl-2">
            ...
            <p className="text-gray-600 text-sm font-normal">
              List of all paid and unpaid properties/cribs bookings available on
              bookacrib
            </p>
          </div>
      ...
      <>
        {loadingadminbooking ? (
          ...
          <div className="relative overflow-x-auto shadow sm:rounded-lg mt-6 ">
            <table className="w-full text-sm text-left rtl:text-right text-gray-600 ">
              ...
              {listadminbookings?.data?.length === 0 ? (
                <tbody>
                  <tr>
                ...
                <tbody>
                  {listadminbookings?.data?.map((item, rowIndex) => (
                    <tr
                      key={rowIndex}
        ...
        <div className="py-5">
          {listadminbookings?.length !== 0 && (
            <Pagination
              meta={listadminbookings?.meta}
