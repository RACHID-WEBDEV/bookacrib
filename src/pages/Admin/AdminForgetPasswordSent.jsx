...
const AdminForgetPasswordSent = () => {
  ...
  const adminEmail = Cookies.get("bac_admin_reset_email")
    ? JSON.parse(Cookies.get("bac_admin_reset_email"))
    : null;
  // console.log(" admin email: ", adminEmail);

  // const adminForgotMessage = Cookies.get("bac_admin_reset_email")
  ...
  const onSubmit = async () => {
    ...
    try {
      const result = await dispatch(
        adminForgetPasswordThunk(adminEmail)
      ).unwrap();
      ...
      if (result.status >= 200 && result.status <= 300) {
        ...
        Cookies.remove("bac_admin_reset_email");
        // navigate("/");
      }
    } catch (error) {
          ...
          <div className=" border bg-gray-50 dark:border-gray-700 dark:bg-gray-800 px-4 lg:px-6 py-8 sm:py-10">
            ...
            <h3 className="text-sm pt-3 text-gray-500 ">
              ...
              <span className=" font-semibold text-gray-800">
                {adminEmail?.email === null || adminEmail?.email === undefined
                  ? "your email"
                  : adminEmail?.email}
              </span>
...
export default AdminForgetPasswordSent;
