...
const RegisterSent = () => {
  ...
  const userSignUpEmail = Cookies.get("bac_user_signup_email")
    ? JSON.parse(Cookies.get("bac_user_signup_email"))
    : null;
  // console.log(" admin email: ", userSignUpEmail);

  const userSignUpSuccessMessage = Cookies.get("bac_user_signup_message")
    ? JSON.parse(Cookies.get("bac_user_signup_message"))
    : null;
  console.log(" userSignUpSuccessMessage:", userSignUpSuccessMessage);
  ...
  const onSubmit = async () => {
    ...
    try {
      const result = await dispatch(
        resendVerificationThunk(userSignUpEmail)
      ).unwrap();
      ...
      if (result.status >= 200 && result.status <= 300) {
        ...
        Cookies.remove("bac_user_signup_email");
        Cookies.remove("bac_user_signup_message");
        // navigate("/");
      }
    } catch (error) {
          ...
          <div className=" border bg-gray-50 dark:border-gray-700 dark:bg-gray-800 px-4 lg:px-6 py-8 sm:py-10">
            ...
            <h3 className="text-sm pt-3 text-gray-500 ">
              ...
              <span className=" font-semibold text-gray-800">
                {userSignUpEmail?.email === null ||
                userSignUpEmail?.email === undefined
                  ? "your email"
